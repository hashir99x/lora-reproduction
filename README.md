...
|---|---|---|---|
| Trainable parameters | *(fill in)* | *(fill in)* | LoRA: orders of magnitude fewer |
| % of total params trainable | *(fill in)* | *(fill in)* | LoRA: typically <1% |
| Validation accuracy (MRPC) | *(fill in)* | *(fill in)* | LoRA should be competitive with full FT |
| Validation F1 (MRPC) | *(fill in)* | *(fill in)* | LoRA should be competitive with full FT |
| Training time | *(fill in)* | *(fill in)* | Not directly compared in the paper (memory is the paper's main efficiency claim, not wall-clock time) |
...
*(Fill in from `results/training_log.json` -> `hardware` after running on
Colab — GPU model, CUDA availability, etc.)*
